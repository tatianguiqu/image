...
public class KDTree {
	...
	private static class Caculate {
		 ...
		 * @return 方差
		 */
		static double var(ArrayList<ImageKey> data, int dimension) {
			double vsum = 0;
			double sum = 0;
		 ...
		 * @return
		 */
		static double mid(ArrayList<ImageKey> data, int dimension) {
			...
			return findPoint(d, 0, i - 1, i / 2);
			
		}

		static double[][] maxAndMin(ArrayList<ImageKey> data, int dimensions) {
		...
		static double distance(double[] nodeA, double[] nodeB) {
			double sum = 0;
			for (int i = 0; i < nodeA.length; i++) {
				sum += (nodeA[i] - nodeB[i]) * (nodeA[i] - nodeB[i]);
			}
			return sum;
		}
		 ...
		 */
		static double minDistance(double[] nodeA, double[] max, double[] min) {
			double sum = 0;
			for (int i = 0; i < nodeA.length; i++) {
				if (nodeA[i] > max[i])
					sum += Math.pow(nodeA[i] - max[i], 2);
				else if (nodeA[i] < min[i]) {
					sum += Math.pow(min[i] - nodeA[i], 2);
				}
			}
		 ...
		 * @return
		 */
		private static double findPoint(double[] data, int low, int high, int point) {
			int lowt = low;
			int hight = high;
			...
			if (low > point) {
				return findPoint(data, lowt, low - 1, point);
			}
			...
			return findPoint(data, low + same.size() + 1, hight, point);
		}
	}
	...
	private void buildDetail(Node node, ArrayList<ImageKey> data, int dimentions) {
		...
		// 选择分割的值
		node.divValue = Caculate.mid(data, node.divDimension);
		double[][] maxmin = Caculate.maxAndMin(data, dimentions);
		node.min = maxmin[0];
		node.max = maxmin[1];
		...
		for (ImageKey image : data) {
			double[] d = image.getImageKey();
			if (d[node.divDimension] < node.divValue) {
				left.add(image);
			} else {
				right.add(image);
			}
	...
	private void printRec(Node node, int lv) {
		if (!node.isLeaf) {
			for (int i = 0; i < lv; i++)
				System.out.print("--");
			System.out.println(node.divDimension + ":" + node.divValue);
	...
	public ArrayList<String> query(double[] input) {
		...
		while (!node.isLeaf) {
			if (input[node.divDimension] < node.divValue) {
		 ...
		 */
		double distance = Caculate.distance(input, node.value);
	...
	public ArrayList<String> queryRec(double[] input, double distance, Stack<Node> stack) {
		...
		while (stack.size() != 0) {
			node = stack.pop();
			if (node.isLeaf) {
				tdis = Caculate.distance(input, node.value);
				if (tdis < distance) {
					distance = tdis;
					nearest = node.path;
				}
				 ...
				 */
				double mindistance = Caculate.minDistance(input, node.max, node.min);
	...
	public ArrayList<String> queryP(double[] input, int k) {
		...
		while (!node.isLeaf) {
			double d = Math.abs(input[node.divDimension] - node.divValue);
			if (input[node.divDimension] < node.divValue) {
		...
		while (!maxHeap.isEmpty()) {
			//System.out.println(maxHeap.getMax().getDistance());
			result.add(maxHeap.deleMax().getPath());
		}
		return result;
	}

	public void queryRecP(double[] input, double distance, Queue<NodeP> priorityQueue, MaxHeap<ImageForSort> maxHeap,
			int k) {
		Node node = null;
		double tdis;
		int countNow=count;
		while (priorityQueue.size() != 0&&countNow!=0) {
			countNow--;
	...
	public ArrayList<ImageForSort> queryPM(double[] input, double mDistance) {
		...
		queryRecPM(input, mDistance, priorityQueue, ifsList);
	...
	public void queryRecPM(double[] input, double mDistance, Queue<NodeP> priorityQueue,
			ArrayList<ImageForSort> ifsList) {
		Node node = null;
		double tmpDis;
		while (priorityQueue.size() != 0) {
			node = priorityQueue.poll().node;
			if (node.isLeaf) {
				tmpDis = Caculate.distance(input, node.value);
				if (tmpDis < mDistance) {
					for (int i = 0; i < node.path.size(); i++) {
						ImageForSort imF = new ImageForSort();
						imF.setDistance(tmpDis);
						imF.setPath(node.path.get(i));
						ifsList.add(imF);
					}
